...
**AniMediRec（動物健康管理アプリ）** は、保護施設・動物病院・多頭飼育の現場向けに、
...
- **Backend**: Firebase（Authentication / Firestore / Cloud Storage / Hosting）。
  **Spark プラン（無料枠）のみで運用する前提**。Cloud Functionsは使わない
  （詳細は「決着済み事項の索引」参照）
...
### 実装しないと決めたもの
- **Cloud Functions の利用**＝Sparkプラン(無料・カード登録不要)のみで運用する条件と
  相反するため不採用（2026-09-11 ユーザー選定）。クライアント側処理＋Firestore/Storage
  のセキュリティルールのみで完結させる。
...
18. **新しい機能を追加して問題なく動作したら、（プロジェクトの運用に合わせて）マイナーバージョンを
    ...
    - **このアプリでの注意**: このセッションでは実機のFirebase環境が無く、
      ログイン・デプロイ等の実際の動作確認ができていない（`docs/specs.md`参照）。
      **「問題なく動作した」の確認は、ユーザーが実際にFirebase環境で試した結果を
      待つ必要がある**（自動検証＝構文チェック・ID整合性チェックだけでは確認手段として
      不十分）。
...
Firebase側の実際の動作（ログイン・Firestore読み書き・Storageアップロード）は、
このセッションでは実機環境が無いため検証できていない。**ユーザーが実機のFirebase
プロジェクトで確認するまでは「未検証」として扱う**（ルール18とも連動）。
...
- **Cloud Functionsは使わない**（無料枠・カード登録不要という前提条件のため。
  「決着済み事項の索引」参照）。サーバーサイド処理が必要に見える要件が出てきたら、
  まずクライアント側＋セキュリティルールで実現できないかを検討し、それでも
  不可能な場合はユーザーに方針転換（Blazeプランへの切り替えが必要になる旨）を
  確認してから進める。
- **写真は必ずクライアント側で圧縮してからアップロードする**（`compressImage()`。
  長辺1280px・JPEG品質0.7）。この処理を経由しないアップロード経路を新設しないこと
  （Storage無料枠5GBを守るための必須ルール）。
- このセッションでは実機のFirebaseプロジェクトが無いため、**ログイン・デプロイ等の
  実際の動作確認は未実施**。ユーザーが`docs/SETUP-FIREBASE.md`の手順で環境を構築した
  後の実機フィードバックを踏まえて調整が必要になる可能性がある。
