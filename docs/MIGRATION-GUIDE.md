...
対象読者: プログラミング未経験で、このアプリの管理を引き継ぐ担当者。
...
これ以降のメンテナンス（機能追加・不具合修正）は、引っ越し先の環境で担当者自身が行う前提。
...
> プランの中でも引き続き無料枠として提供される。** 動物数十頭程度のご利用規模
> （写真は圧縮済み）であれば、この無料枠を超えることはまず考えられない。ただし想定外の
...
> なら、5GBに達するまでに約2万枚必要な計算になり、通常のご利用であれば当面心配は
> 不要。
>
> **（正確には）月末時点の残量ではなく、月内の平均保存量で判定される**: Googleの
> 公式説明では、保存料金は秒単位で日割り計算される。つまり「月末に5GB未満まで
> 減らしたか」ではなく、「その月ずっとどれだけ保存していたかを平均した値」で
> 決まる。例えば月の前半は4GB、後半に写真を追加して6GBになった場合、平均は
> ちょうど5GB-月になり無料枠内におさまる。逆に、月末だけ帳尻を合わせて5GB未満に
> 減らしても、それまでの期間ずっと超過していた分はきちんと課金される。実務上は
> 「合計保存量が5GBを超えたら、超えた分だけ課金され始める」という理解で十分。
>
...
どの項目も無料の範囲の**3割以下**で、通常のご利用であれば日常的に気にされる必要はない。
